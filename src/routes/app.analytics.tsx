            ...
            <thead>
              <tr className="border-b border-border/50">
                {["Agent", "Role", "Calls", "CSAT", "Resolution Rate", "Performance"].map((h) => (
                  <th
                    key={h}
                    className="pb-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground first:pl-0"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-border/30">
              {agentPerf.map((a) => (
                <tr key={a.name} className="group transition hover:bg-muted/20">
                  ...
                  <td className="py-3">
                    <Badge
                      className={`text-[10px] ${a.resolution >= 90 ? "bg-success/12 text-success border-success/20" : "bg-warning/12 text-warning border-warning/20"}`}
                    >
