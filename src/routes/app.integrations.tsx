  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Connect your favorite tools — your agent uses them mid-call.</p>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {items.map((i) => (
          <Card key={i.name} className="glass p-4">
            <div className="flex items-start justify-between">
              <div className="grid h-10 w-10 place-items-center rounded-xl bg-primary/15 text-primary"><Puzzle className="h-5 w-5" /></div>
              {i.connected
            ...
            <Button size="sm" variant={i.connected ? "outline" : "default"} className="mt-3 w-full">{i.connected ? "Manage" : "Connect"}</Button>
          </Card>
        ))}
      </div>
    </div>
