  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Every inbound, outbound, and missed call.</p>
      </div>

      <Card className="glass p-5">
        <div className="mb-4 flex items-center gap-3">
          <div className="relative max-w-md flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Search caller, agent, outcome…" className="pl-9" /></div>
        </div>
          ...
          <tbody>
            {logs.map((l) => (
              <tr key={l.id} className="border-b border-border/40 last:border-0 hover:bg-muted/30">
                ...
                <td className="p-3"><Badge variant="outline">{l.outcome}</Badge></td>
